...
data <- read_csv(file = "PPI_match.csv")
#######计算CC/MF correlation
...
for (i in 1:nrow(data)) {
  sim1 <- GOSemSim::geneSim(as.character(data[i,3]),as.character(data[i,4]), semData=hsGO1, measure="Wang", drop=NULL, combine="BMA")
  sim2 <- GOSemSim::geneSim(as.character(data[i,3]),as.character(data[i,4]), semData=hsGO2, measure="Wang", drop=NULL, combine="BMA")
  sim3 <- GOSemSim::geneSim(as.character(data[i,3]),as.character(data[i,4]), semData=hsGO3, measure="Wang", drop=NULL, combine="BMA")
...
write.csv(data, file = "PPI_match_score.csv")
